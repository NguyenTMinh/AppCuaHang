...
public class PanelStartApp extends BasePanel{
	...
	public void addComp() {
		...
		add(lb_AppName);
		lb_BanLa = creatLabel(BAN_LA, 0, lb_AppName.getY()+lb_AppName.getHeight()+40, font2, Color.BLACK, getBackground());
		x = GUI.W_SIZE/2 - lb_BanLa.getWidth()/2;
		lb_BanLa.setLocation(x, lb_BanLa.getY());
		add(lb_BanLa);
		bt_CuaHang = creatButton(CH, 0, lb_BanLa.getY()+lb_BanLa.getHeight()+20, font2, Color.BLACK, BT_CUAHANG);
		x = GUI.W_SIZE/2 - bt_CuaHang.getWidth()/2;
		bt_CuaHang.setLocation(x,bt_CuaHang.getY());
		add(bt_CuaHang);
		bt_KhachHang = creatButton(KH, 0, bt_CuaHang.getY()+bt_CuaHang.getHeight()+20, font2, Color.BLACK, BT_KHACHHANG);
		...
		add(bt_KhachHang);
		lb_DangKy = creatLabel(TEXT_DANG_KY, 0, bt_KhachHang.getY()+bt_KhachHang.getHeight()+40, font2, Color.BLACK, getBackground());
		x = GUI.W_SIZE/2 - lb_DangKy.getWidth()/2;
		lb_DangKy.setLocation(x, lb_DangKy.getY());
		add(lb_DangKy);
		bt_DangKy = creatButton(DANG_KY, 0, lb_DangKy.getY()+lb_DangKy.getHeight()+20, font2, Color.BLACK, BT_DANGKY);
